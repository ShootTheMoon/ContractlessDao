...
class Web3Service {
    ...
    constructor(rpcUrl) {
        ...
        this._contract = new ethers_1.ethers.Contract(process.env.TOKEN_ADDRESS, abi, this._provider);
    }
    /**
    ...
    init() {
        ...
        setInterval(() => {
            ...
            if (lastScanned && (Date.now() - lastScanned.getTime()) < VOTE_SCAN_INTERVAL && isScanning)
                return;
            this._getTokenBalancesForActiveProposals();
...
exports.default = new Web3Service(process.env.JSON_RPC_URL);
